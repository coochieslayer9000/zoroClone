...
const Card = ({ anime }) => {
  if (!anime) return <div className="w-full h-64 bg-gray-800 animate-pulse rounded-lg"></div>;
  ...
  return (
    <Link to={`/watch/${anime.id}`} className="block group relative overflow-hidden rounded-lg shadow-lg hover:shadow-2xl transition-all duration-300 bg-gray-800">
      <img
        ...
        onError={(e) => { e.target.src = 'https://via.placeholder.com/200x300?text=No+Image'; }} // Fallback if API fails
      />
